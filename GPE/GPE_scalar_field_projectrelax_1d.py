...
class GPE_scalar_field_projectrelax_1d:
    ...
    def sum_contributions(self,dt,t,*args):
        ...
        if (self.relax):
            ...
            proj_psi = psi_new*np.sqrt(inv_list_old[0]/mass_new)
            ...
            psi_new = self.psi*(1.0-self.rel_gamma) + proj_psi*self.rel_gamma
            mass_new = np.mean(np.conj(psi_new)*psi_new).real
            psi_new = psi_new * np.sqrt(inv_list_old[0]/mass_new)
