...
class GPE_scalar_field_multirelax_test_1d:
    ...
    def sum_contributions(self,dt,t,*args):
        ...
        if (self.relax):
            ...
            sol = root(self.func2optimize,self.gamma_0,args=(psi_new,terms,inv_list_old,dt,t,*args), method='hybr',tol=1e-14,options={"maxfev":2000})
            ...
            if not(sol.success) and np.max(np.abs(sol.fun))>1e-10:
               ...
               # self.rel_gamma = self.gamma_0
                #print(show_options(solver="root",method="hybr"))

                #sys.exit("Exiting due to non-convergence in relaxation step")
                psi_new = self.psi
                self.f = 1.0*self.psi
                return True
            else:
            ...
            psi_new = psi_new + np.einsum("i,ij->j", self.rel_gamma, terms)
