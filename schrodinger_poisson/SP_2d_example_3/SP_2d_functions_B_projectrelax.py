...
from GPE import GPE_scalar_field_projectrelax
...
def run_SP_2d_example_1(energy_type,dt,X,Xi,kppa,t_ini,T,L,imx,inv_list,u_ini,exact_soln_np=None,dx_soln_jx=None,relax=False,log_errs=False,lap_fac=1.0,num_plots=100,p=3.0,
    ...
    def func2optimize_SP_1(rel_gamma,u,proj_u,inv_list_old,dt,t,*args):
               # print(rel_gamma.shape,terms.shape,np.dot(rel_gamma,terms).shape)
                u_gamma = u*(1.0-rel_gamma)+ proj_u*rel_gamma

                mass_new = np.mean(np.abs(u_gamma)**2).real
                ...
                Ek_new = kin_energy_1(proj_u_gamma,*args)
                Ev_new = pot_energy_1(proj_u_gamma,*args)
                Q_new = p_new*(Ek_new)-q_new*(Ev_new)
                    #print(p,q,p_new,q_new,Q_old,Q_new)

                inv_func = np.array(Q_new-Q_old-inv_list_old[-1])
          
                    #inv_func = np.array([mass_new-inv_list_old[0],p*(Ek_new-inv_list_old[1])-q*(Ev_new-inv_list_old[2])])
                
                return inv_func
    
    def func2optimize_SP_2(rel_gamma,u,proj_u,inv_list_old,dt,t,*args):
               # print(rel_gamma.shape,terms.shape,np.dot(rel_gamma,terms).shape)
                u_gamma = u*(1.0-rel_gamma)+ proj_u*rel_gamma
                ...
                Ek_new = kin_energy_2(proj_u_gamma,*args)
                Ev_new = pot_energy_2(proj_u_gamma,*args)
                Q_new = p_new*(Ek_new)-q_new*(Ev_new)
                    #print(p,q,p_new,q_new,Q_old,Q_new)
    ...
    if relax==2:
        rhoq = GPE_scalar_field_projectrelax(2,m,rhs_linear,rhs_nonlinear,imx,u_ini,relax=2,conserve_list=[mass,rel_kin_energy,rel_pot_energy,Q_dummy],func2optimize=func2optimize_SP)
    elif relax==1:
        rhoq = GPE_scalar_field_projectrelax(2,m,rhs_linear,rhs_nonlinear,imx,u_ini,relax=1,conserve_list=[mass],func2optimize=func2optimize_SP)
    else:
        rhoq = GPE_scalar_field_projectrelax(2,m,rhs_linear,rhs_nonlinear,imx,u_ini)
    ...
    while (t<=tmax):
        ...
        if (np.mod(n,nplt)) == 0 :
            ...
            print("Mass",mass_now,mass_relative_change,energy_violation,np.abs(eq_E_fd))
