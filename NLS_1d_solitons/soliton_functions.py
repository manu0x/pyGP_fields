...
from GPE import GPE_scalar_field_multirelax_test_1d
...
def kin_energy_0(u,xi,kppa,lap_fac,mu,Q=None):
        ...
        ##################################################### EP 0 #################################
        E_k0 = np.sum(xi*xi*np.abs(u_ft)**2)
...
def run_SP_2d_example_1(dt,X,Xi,kppa,t_ini,T,L,imx,inv_list,u_ini,kin_type=None,exact_soln_np=None,dx_soln_jx=None,relax=False,log_errs=False,lap_fac=1.0,num_plots=100,p=3.0,
    ...
    def func2optimize_SP(rel_gamma,u,terms,inv_list_old,dt,t,*args):
               # print(rel_gamma.shape,terms.shape,np.dot(rel_gamma,terms).shape)
                u_gamma = u + np.einsum("i,ij->j", rel_gamma, terms)
                ...
                if relax>1:
                    ...
                    inv_func = np.array([mass_new-inv_list_old[0],Q_new-Q_old])
    ...
    if relax==2:
        rhoq = GPE_scalar_field_multirelax_test_1d(1,m,rhs_linear,rhs_nonlinear,imx,u_ini,relax=2,conserve_list=[mass,kin_energy,pot_energy],func2optimize=func2optimize_SP)
    elif relax==1:
        rhoq = GPE_scalar_field_multirelax_test_1d(1,m,rhs_linear,rhs_nonlinear,imx,u_ini,relax=1,conserve_list=[mass],func2optimize=func2optimize_SP)
    else:
        rhoq = GPE_scalar_field_multirelax_test_1d(1,m,rhs_linear,rhs_nonlinear,imx,u_ini)
    ...
    while (t<=tmax):
        ...
        #print("Q_int:", Q_int)
        retry = rhoq.sum_contributions(dt,t,xi,kppa,lap_fac,mu,Q_int)
        
        if retry:
             dt = dt*0.5
             print("changing dt to ",dt)
        elif relax:
        ...
        if (np.mod(n,nplt)) == 0 :
            ...
            if relax:
                 print("Time ",t," step ","Gamma sum",n,np.sum(rhoq.rel_gamma))
            else:
